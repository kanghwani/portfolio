---
name: 김강환 포트폴리오 디자인 시스템
description: 현재 공개판의 우주 작업실과 보관판의 인쇄 매뉴얼 규칙
colors:
  ink: "#191714"
  ink-soft: "#3A362F"
  milk: "#F6F3EC"
...
# 현재 공개판: 월넛 작업실 (2026-09-09)

## 승인 디자인 — 책상 (2026-09-13)

- 09-13 사용자 승인으로 새 책상을 기본 URL에 적용한다. `/v2/3d/?desk=compare`에서만 `기존 / 변경안` 비교 패널을 보여준다. 새 모델 로드 실패 시 이전 책상 유지.
- 기존 조명·카메라·모니터·소품·본문은 고정. 책상은 바닥 마루 패턴 대신 Blender에서 구운 전용 목재 텍스처, 얇은 앞단, 작은 모서리 곡률, 서랍 상부 지지대로 개선한다.
- 상판 높이 0.7775m와 2.4×1.1m 면적을 보존한다. 피규어 착지 좌표와 기존 서랍은 변경하지 않는다.
- 팔레트: 나뭇결 `#75614F / #86705B / #97816B`, 하부 `#6F5946`, 금속 `#3D3934`. 기존 Pretendard/Galmuri 및 본문 레이아웃 유지.
- 원본 `workstation.blend/.glb`는 동결. 승인 모델은 `desk-refined.blend/.glb`, 생성기는 `tools/blender/build_desk_refinement.py`. 다음은 모니터만 별도 비교하며 승인 전 기본 화면에 반영하지 않는다.

개발 `/v2/3d/`·QA `/qa/`·기획 `/design/`는 `assets/workspace-tokens.css`를 공유한다. 현재 토큰은 월넛 `#211A16`, 표면 `#2D241E`, 종이 `#F3EADF`, 보조 `#C2B2A0`, 앰버 `#E3A257`, 경계 `#665346`이다. 본문 Pretendard, 짧은 식별자 Galmuri11.

- 개발: Blender 책상·CRT, 부팅 완료 전 클릭 차단·느린 풀백. 코드 타이핑 중간 단계와 모니터 속 진입 연출은 제거. 피규어는 클릭하면 책상으로 점프.
- QA: 같은 재질의 입체 기록 카드·대조군 선택·보고서 탐색. 내용은 평면 고정. 포인터 기울기는 클릭 좌표를 움직여 제거했고 정적 원근만 유지한다. 게임 재현이 아닌 과거 관측 기록 탐색임을 표시.
- 기획: 기존 명세 시각화·수치 모델·반증 조건 보존. 배경·리프·타이포만 같은 작업실 물성으로 동기화. 가정치와 미구현·미실측 표시는 남긴다(09-11: 포괄적인 작성 중 대신 정확한 제안 범위 명시).
- `/v1/`·`/111/`의 보관/제출본은 새 테마에서 제외한다.

아래 「우주 작업실」 규칙과 수치는 **09-01 당시 설계 이력**이다. 충돌하면 위 현재 토큰과 실제 공통 CSS가 우선한다.

# 이전 공개판: 우주 작업실 (09-01 이력)

## 쓰임

`/v2/3d/`는 Unity 클라이언트 개발자의 작업 과정을 보여주는 현재 공개판입니다. 첫 화면의 역할은
게임 장면을 대신해 코드를 쓰고 테스트하는 사람을 보여주는 것입니다. 본문에서는 그 인상을 실제 코드,
측정 수치, 실패 기록으로 확인합니다.

## 시각 방향

- 우주에 떠 있는 작은 작업실을 한 장면으로 구성합니다. CRT 모니터, 키보드, 개발 보드, 책, 듀얼센스와 거치대처럼 실제 작업을 설명하는 물건만 둡니다.
- 움직임은 첫 화면에 모읍니다. C# 코드가 한 글자씩 입력되고 카메라가 뒤로 빠지며 책상 전체를 보여줍니다.
- 본문은 진단 로그처럼 읽혀야 합니다. 프로젝트 설명을 반복 카드로 쪼개지 않고 제목, 기록, 코드, 측정값이 자연스럽게 이어지게 합니다.
- 장식용 번호와 근거 없는 강조는 쓰지 않습니다. 색은 현재 위치와 중요한 상태를 알릴 때만 사용합니다.
...
| CRT 외장 | 차가운 회색 계열 |
| 본문 글꼴 | Pretendard 계열 |
| 시스템 글꼴 | SFMono-Regular, Consolas, monospace |

## 기억할 장면

회색 CRT 화면에 Unity C# 코드가 입력됩니다. 카메라가 빠지면 테스트 러너, 개발 보드,
거치대에 세운 듀얼센스가 함께 보입니다. 이 장면이 현재 공개판의 시그니처입니다.

---

# 보관판: 박스 소프트웨어 사용자 매뉴얼
