...
> **경계(boundary): 이 시스템은 `index.html`(개발 포트폴리오)의 월드만 규정한다.**
> `design/`과 `qa/` 하위 페이지는 각자의 기존 스타일을 따로 갖고 있으며 이 월드에 속하지 않는다.
> 새 화면이 이 파일을 상속하는 범위는 index.html의 세계 안에서만이다.
...
- **Don't** `design/`·`qa/` 하위 페이지에 이 시스템을 강제하지 마라 — 경계 밖이다.
